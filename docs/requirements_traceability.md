...
| FR-07 | "Produce actionable exception tables with stable identifiers, severity, and resolution context." | US-04 | `dbt/models/marts/finance/mart_finance_exceptions.sql` | `tests/test_excel_workbook.py::test_exception_exposure_counts_each_invoice_once` | Met |
...
| FR-09 | "Estimate unit economics with transparent assumptions and sensitivity ranges." | US-08 | `dbt/models/marts/growth/mart_channel_efficiency.sql`, `docs/analytical_design.md` | `tests/modeling/test_churn.py::test_threshold_sensitivity_economics_are_explicit` | Met, assumptions labeled |
| FR-10 | "Build time-aware churn models with calibration, lift, explainability, and a cost-based threshold." | US-06, US-07 | `src/modeling/churn.py` | `tests/modeling/test_diagnostics.py::test_temporal_split_has_no_overlap`, `tests/modeling/test_churn.py::test_threshold_maximizes_stated_expected_value` | Met, weak model reported honestly |
